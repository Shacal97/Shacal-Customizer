...
ctx.SHACAL_SCRIPT_VERSION = '6.11.6';
...
ctx.checkForShacalUpdate = async function checkForShacalUpdate(options = {}) {
        ...
        ctx.shacalUpdateCheckPromise = (async () => {
            try {
                ...
                ctx.shacalUpdateState.error = null;
                if (ctx.shacalUpdateState.available) window.ShacalRuntime?.showUpdateNotice?.(remoteVersion);
                return {
...
declare(ctx){
    const keys={
        ...
        chat:['addon_chat','chatAnnouncementsEnabled','chatMessageTemplate','chatEmoticonsEnabled','chatProfanityFilterEnabled'],
        ...
        godmode:['addon_godmode','godModeEnabled']
    };
